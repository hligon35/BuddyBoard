  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f3f4f6' }}>
      <View style={styles.inputTileCompact}>
        <TouchableOpacity onPress={onAttachPress} style={styles.iconButton} accessibilityLabel="Attach photo or link">
          <MaterialIcons name="attach-file" size={20} color="#0066FF" />
        </TouchableOpacity>
        ...
        <TextInput
          ...
          style={[styles.inputTextCompact, styles.inputFlex]}
          placeholderTextColor="#9ca3af"
          multiline
        />

        <TouchableOpacity style={styles.iconBoxPrimary} onPress={handlePost}>
      ...
      {image ? (
        <View style={{ paddingHorizontal: 12, marginTop: 8 }}>
          ...
          <Modal visible={showLinkModal} transparent animationType="fade" onRequestClose={() => { setShowLinkModal(false); setLinkMode(false); }}>
            <View
              style={styles.modalOverlay}
              onStartShouldSetResponder={() => true}
              onResponderRelease={() => { setShowLinkModal(false); setLinkMode(false); }}
            >
                ...
                {!linkMode ? (
                  <View style={{ flexDirection: 'row', marginTop: 6 }}>
                    <TouchableOpacity style={styles.modalBtn} onPress={pickImage}>
                      <Text style={{ color: '#0066FF', fontWeight: '700' }}>Photo</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.modalBtn} onPress={() => setLinkMode(true)}>
                      <Text style={{ color: '#0066FF', fontWeight: '700' }}>Link</Text>
                    </TouchableOpacity>
                  </View>
                ) : (
                  <>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginTop: 10 }}>
                      <TouchableOpacity onPress={() => { setLinkMode(false); setLinkInput(''); }} style={{ marginRight: 10 }}>
                        <Text style={{ color: '#374151' }}>Back</Text>
                      </TouchableOpacity>
                      <TouchableOpacity onPress={() => { if (linkInput) { setBody((b) => (b ? b + '\n' + linkInput : linkInput)); setLinkInput(''); } setLinkMode(false); setShowLinkModal(false); }}>
                        <Text style={{ color: '#0066FF', fontWeight: '700' }}>Add</Text>
                      </TouchableOpacity>
                    </View>
                  </>
                )}
          ...
          <Modal visible={shareModalVisible} transparent animationType="fade" onRequestClose={() => { setShareModalVisible(false); setShareTargetPost(null); }}>
            <View style={styles.modalOverlay} onStartShouldSetResponder={() => true} onResponderRelease={() => { setShareModalVisible(false); setShareTargetPost(null); }}>
              <TouchableWithoutFeedback onPress={() => { /* swallow inner taps */ }}>
                <View style={styles.modalContent}>
                  <Text style={{ fontWeight: '700', marginBottom: 8 }}>Send post</Text>
                  <TouchableOpacity style={styles.modalOption} onPress={() => handleShareViaMessages(shareTargetPost)}>
                    <Text style={{ color: '#0066FF', fontWeight: '700' }}>Messages</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.modalOption, { marginTop: 8 }]} onPress={() => handleShareViaEmail(shareTargetPost)}>
                    <Text style={{ color: '#0066FF', fontWeight: '700' }}>Email</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.modalOption, { marginTop: 8 }]} onPress={() => handleShareMore(shareTargetPost)}>
                    <Text style={{ color: '#0066FF', fontWeight: '700' }}>More...</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.modalOption, { marginTop: 12, backgroundColor: '#fff' }]} onPress={() => { setShareModalVisible(false); setShareTargetPost(null); }}>
      ...
      <FlatList
        ...
        keyExtractor={(i) => `${i.id}`}
        renderItem={({ item }) => (
          <PostCard post={item} onLike={(p) => like(p.id)} onComment={(p) => navigation.navigate('PostThread', { postId: p.id })} onShare={(p) => share(p.id)} />
        )}
